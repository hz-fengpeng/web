...
JS 写起来自由，但代价是"写错了要等运行到那一行才发现"。
[`ts_learn/`](../ts_learn/) 就是补上这一层：同样的代码，把类型标出来，
让错误在**编辑器里**就暴露出来，而不是在用户手里。

先把这个目录的 1～6 章过一遍再去看它 —— TypeScript 加的是"类型"，
但语法、运行时行为、异步模型、DOM 操作全都是 JS 原来那套，
底子不牢的话会变成在背类型语法。
